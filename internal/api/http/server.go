...
type actorLoader interface {
	...
	ByApiKey(token string) (*dto.Actor, error)
}
...
func configureDefaultSecurityRequirements(api *huma.OpenAPI, op *huma.Operation) {
	...
	for i, req := range op.Security {
		if _, ok := req[authSchemeName]; !ok {
			op.Security[i][authSchemeName] = []string{}
		}
		if _, ok := req[apiKeySchemeName]; !ok {
			op.Security[i][apiKeySchemeName] = []string{}
		}
	}
}
...
func (srv *Server) Initialise(controllers []Controller) {
	...
	srv.echo.Pre(srv.AccessLogger())
	srv.echo.Pre(middleware.Recover())
	...
	apiCfg.Components.SecuritySchemes = map[string]*huma.SecurityScheme{
		...
		"api_key": {
			Type:        "apiKey",
			Description: "API key generated by the application.",
			Name:        "x-api-key",
			In:          "header",
		},
	...
	hg.UseMiddleware(srv.AddOperationIDMiddleware())
...
func NewServer(sessionManager sessionManager, al actorLoader, logger *slog.Logger, accessLogger *slog.Logger) *Server {
	return &Server{
		logger:         logger,
		sessionManager: sessionManager,
		al:             al,
